...
// v2.74.1112 — + IL ▸ (the IL-2 inference-layer loop's per-run decision line: status + step count).
// v2.74.1123 — + GAPS ▸ (PS-0: Orchard's per-Ground capability-gap enumeration, persisted from a meta answer).
// v2.74.1124 — + HARVEST ▸ (PS-1: a user click on an unknown control fulfilled a declared gap → harvested).
// v2.74.1126 — + SYNTH ▸ (PS-3: a harvested gap synthesized into a durable unverified capability, stage/verify-on-first-use).
// v2.74.1138 — + VERSION ▸ (version-at-land: a dev-branch land stamps main's manifest version to current+1, docs/DESIGN_surfaces.md §4.2).
// v2.74.1139 — + REVIEW ▸ (the review gate: a dev run's done auto-offers Approve/Preview/Discard, docs/DESIGN_surfaces.md §4.3).
// v2.74.1140 — + DEPLOY ▸ (Approve-scoped auto-deploy: a review-gate land redeploys main to the live build, docs/DESIGN_surfaces.md §4.4).
// v2.74.1144 — + SURFACE ▸ (keystone K2/K4: the host injects a run's surface altitude preamble at spawn, docs/DESIGN_surfaces.md §2.2).
// v2.74.1172 — + WRITE_GATE ▸ (CV-6: a read-only app/sub-task [writePolicy:'never'] blocked an act, DESIGN_conversations.md §8).
// v2.74.1176 — + INTERPRET ▸ (F-2c: the LLM front-door interpret call's decision — intent + confidence, DESIGN_llm_front_door.md §9).
// v2.74.1205 — + CANVAS ▸ (CA-4: a RENDER_CANVAS write to an app's presentation tab — op + docId + rev + block count, DESIGN_canvas.md §3).
// v2.74.1280 — + FORAGE ▸ (§19: the read-safe nav-following recipe-capture crawl — pages visited / banked, DESIGN_connectors.md §19).
// v2.74.1290 — + SESSION_REPLAY ▸ (§20: header-replay of a harvested cross-origin recipe — apiHost·method·status|noAuth|error, DESIGN_connectors.md §20).
// v2.74.1295 — + ANSWER_GUARD ▸ (honesty belt: a side-effect COMMAND routed to answer but the reply claimed the act was done → neutralized, Core/answerGuard.js).
//              ⚠ ALSO: SESSION_REPLAY ▸ (1290) was documented above but NEVER actually added to the pattern below (an Invariant #1 slip — invisible to gc since 1290) — wired now alongside ANSWER_GUARD.
// v2.74.1299 — + DEMO_WRITE ▸ (CX-8c: a demonstration captured the app's OWN write request → a pending ride write-recipe banked, DESIGN_connectors.md §10 CX-8).
// v2.74.1306 — + CONNECTOR_INVOKE ▸ (CX-5b: the broker/OAuth INVOKE_CONNECTOR outcome — BLOCKED (write-needs-confirm) / ok / broker-unavailable, DESIGN_connectors.md §5).
// v2.74.1310 — + CONNECTOR_LINK ▸ (MP-3: the client-side PKCE link dance outcome — linked / BLOCKED (no client id) / state-mismatch / user-cancel, DESIGN_connectors.md §5.2).
// v2.74.1319 — + CONNECTOR_TOOLS ▸ (MP-2c: live tools/list discovery — linked count · live servers · tool count; the palette then carries server-published schemas, DESIGN_connectors.md §5.2).
// v2.74.1330 — + SOURCE ▸ (GD-7e: a page banked as compose reference material — title · kb: id · media counts · app, DESIGN_canvas.md §8.7.1).
// v2.74.1342 — + WORKFLOW ▸ (WF-1: saved-workflow recall pre-empts interpret); + RIDE_WRITE ▸ (CX-6 HITL write confirm);
// + INTERPRET_ASK " (F-2 SW-side interpret line — candidate count + ground + raw intent).
// v2.74.1346 — + SWEEP ▸ (FL-1: the fleet app's propose-only run — reads picked · proposals minted, DESIGN_app_fleet.md).
// v2.74.1347 — + SHOW ▸ (FL-1c: ground-truth viewing — origin · target count · reused-vs-new tab).
// v2.74.1417 — + LEG_TEST ▸ (OV-4: the Overview workbench's structural leg-test VERDICT — pass/fail + verdict code) and
//              + LEG_VERIFY ▸ (OV-6: a developer ARMED a leg — reviewed accepted + enabled → app-consumable, DESIGN_overview.md §3).
//              (LEG_OVERVIEW ▸ is an inventory READOUT, not a decision — omitted here on the CAPABLE_SITES ▸ precedent.)
// v2.74.1434 — + RIDE_RESOLVE ▸ (CX-9b: a human param value resolved to its canonical id — "Atlanta West"/"210" → 83)
//              + RIDE_DRILL ▸ (CX-9b: a list read's row joined into its details read — via·param=id·the matched filter).
// v2.74.1453 — + SECTION_NAV ▸ (CX-9o: the data-derived navigation leg — host·section·find + the click outcome).
// v2.74.1454 — + DRIVE_HYDRATE ▸ (HL-2: a built-in drive artifact composed into library entities on first use) and
//              + DRIVE_INVOKE ▸ (HL-3: a drive-artifact run — ok/failed · [hydrated] · [promoted trial-pass] · params).
// v2.74.1488 — + RIDE_EACH ▸ (DK-7: an each-mode read fan-out — leg × N enumerated values → ok/failed row totals).
// v2.74.1491 — + ROUTINE ▸ (DK-8: a desk's declared routine — declared/armed/disabled/due/fired · cadence · ask).
// v2.74.1500 — + CASE_BRIEF ▸ (DK-8h: spawned cases reframed in the requestor's voice — N/M framed).
// v2.74.1506 — + CONN ▸ (CP-1: connection-presence transitions — origin from→to (cause) [source] · heartbeat sweeps).
// v2.74.1523 — + LEARNED ▸ (goal-memory lifecycle — act-fail rules retired after a re-teach).
...
// v2.74.1570 — + VITALS ▸ (VT-1/3: the vitals scheduler — presence sweeps, daily visits, canary verdicts, incidents; DESIGN_vitals.md §4).
// v2.74.1583 — + DASH ▸ (VT-2d: the context-determined dashboard — scope + block/ground/run counts per open).
// v2.74.1661 — + BRANCH ▸ (PP-1: the per-item classify-and-route — arm/none/unknown dispositions per run).
// v2.74.1831 — + SPAN ▸ / RUN ▸ (OB-1 observability: SPAN is the PAIRED EXIT emitted from a finally, so an
//              early return can no longer leave a hole — three silent failures this arc were entry-line-only.
//              RUN ▸ is the turn BACKSTOP: touched no artifact AND raised no error. Distinct from "▶ RUN ".)
// v2.74.1828 — + STEP ▸ (the per-step outcome RECEIPT: rows in→out, predicate declared/applied, artifacts
//              created/updated/skipped. The line that makes "I did LESS than I was told" greppable — five
//              silent workflow failures on 07-25 produced ZERO error lines. Distinct from STEPS ▸ (the plan).)
// v2.74.1681 — + WRITE ▸ / UPSERT ▸ (PP-2: the per-item write — gate decision, created/queued/blocked per row).
// v2.74.1669 — + STEPS ▸ (intent → workflow steps: count, the intent-derived floor, under-split/compound flags).
// v2.74.1665 — + PIPELINE ▸ (PP: the run object — start/end lines carrying the run VERDICT, not just counts).
// v2.74.1662 — + REDACT ▸ (R-2/R-3: PII pseudonymized before egress / unresolved pseudonyms in a reply —
//              DESIGN_llm_privacy.md §7. This is the line that proves the privacy boundary actually fired).
const _DECISION_RE = /(▶ RUN |[✓✗] RUN |COMPREHEND_CROSS_GROUND ▸|T3X resolve ▸|T3X bind ▸|_bind ▸|GROUNDS ▸|ROUTE ▸|HANDOFF ▸|postcond ▸|ORCH_MATCH ▸|ORCH_MATCH_GLOBAL ▸|DETECT_DUPLICATE_GROUNDS ▸|MERGE_GROUNDS ▸|mergeGround |Ground saved:|Ground deleted:|→ (?:auto|propose|miss)\/|RUN_OBSERVATION|RUN_BEST_OBSERVATION|ORCH_RECORD_ALIAS|ORCH_ADMIN ▸|REPLAY_SG_CAPABILITY —|— bindings:|CLICK caused navigation|WALK ▸|LOOP ▸|ORCH_PLAN ▸|OPEN_URL_NEW_TAB —|REVERIFY_SG_CAPABILITY —|ROUTE_ASK "|bindClauseParams →|locale-fresh-skip|locale-trust:|EXPLORE_PAGE_STRUCTURE done|RUN_SG_TRIAL|INTERACTION_MONITOR_START|INTENT_MENU ▸|RICH_INTENTS ▸|ACCEPT_SG_TRIAL|INTERACTION_OUTCOMES ▸|proposeRichIntents —|ensureGroundForUrl|EXPLORE ▸|STOP ▸|FOCUS ▸|CLARIFY ▸|CLOSE_TABS ▸|DEVBR ▸|LT ▸|CONCERN ▸|SYNC ▸|MERGE ▸|ABANDON ▸|DRIFT ▸|SPLIT ▸|FORK ▸|SCOPE ▸|WORKTREE ▸|MERGE_LOCK ▸|VERSION ▸|REVIEW ▸|DEPLOY ▸|SURFACE ▸|IL ▸|GAPS ▸|HARVEST ▸|SYNTH ▸|WRITE_GATE ▸|INTERPRET ▸|CANVAS ▸|FORAGE ▸|SESSION_REPLAY ▸|ANSWER_GUARD ▸|DEMO_WRITE ▸|CONNECTOR_INVOKE ▸|CONNECTOR_LINK ▸|CONNECTOR_TOOLS ▸|SOURCE ▸|WORKFLOW ▸|CADENCE ▸|TRIGGER ▸|RIDE_WRITE ▸|RIDE_RESOLVE ▸|RIDE_DRILL ▸|RIDE_EACH ▸|ROUTINE ▸|CASE_BRIEF ▸|CONN ▸|SECTION_NAV ▸|DRIVE ▸|DRIVE_HYDRATE ▸|DRIVE_INVOKE ▸|INTERPRET_ASK "|INTERPRET_ASK ▸|PALETTE ▸|IDENTITY_PROBE ▸|SWEEP ▸|SHOW ▸|LEG_TEST ▸|LEG_VERIFY ▸|LEARNED ▸|FIELD_FOLLOWUP ▸|OBS_PARAM ▸|TARGET ▸|INVOKE ▸|HEAL ▸|VITALS ▸|DASH ▸|MAP ▸|FIELD_READ ▸|CONTEXT ▸|PAYLOAD ▸|FIND ▸|DISPATCH ▸|CLAUSE_ERROR ▸|BRANCH ▸|REDACT ▸|PIPELINE ▸|GATE   ▸|STEPS ▸|STEP ▸|SPAN ▸|RUN ▸|READ   ▸|TASK   ▸|PRESENCE ▸|PANEL ▸|CHILDREN ▸|RIDE_TAB ▸|WRITE ▸|UPSERT ▸|CASE   ▸)/;
