...
OUTPUT: Return ONLY the raw JSON array. No fences, no explanation. {{USER_QUESTION}} for TYPE value. Sequential step numbers starting at 1.`;
  ...
  static async interpret({ ask, retrieved, primitives, affordances, seed, target, connections, learned, objects, subTasks, history } = {}) {
    ...
    try {
      ...
      _now = `${new Date().toLocaleString('sv-SE').slice(0, 16)} (${_tz}) = ${new Date().toISOString().slice(0, 16)}Z`;   // v1920 — BOTH forms: local for phrasing ("tomorrow 3pm", the v1317 purpose), UTC for delta math against Z-stamped record fields
  ...
  static async sweepPropose({ seed = '', learned = '', objects = '', legs = [], askLegs = [], results = [], round = 1, context = '', evidence = '', issues = [] } = {}) {
    if (!(await AnthropicService.hasLlm())) return null;
    const { system, user } = buildSweepProposeMessages({ seed, learned, objects, legs, askLegs, results, round, context, evidence, issues });
