...
export const ConversationStore = {
  ...
  async patchMeta(id, fields = {}) {
    ...
    for (const k of ['branch', 'concern', 'sessionId', 'status', 'mergedAt', 'mergeCommit', 'title', 'syncedMain', 'seed', 'titledByLlm', 'surface', 'appId', 'appVersion', 'parentId', 'icon', 'config', 'pinned', 'instanceId', 'presetId', 'focus', 'summary', 'resolvedAt']) {   // .1102 — titledByLlm; .1147 — surface (the dev altitude; WITHOUT this, `surface high` silently dropped the field and never activated the high surface); .1168 (CV-3c) — app identity (appId/appVersion/parentId/icon) + tighten-only config; .1211 (AP-0/1) — pinned + per-instance identity (instanceId/presetId); .1552 (FC-0) — focus (the conversation's working set; accretion/refresh patch it); .1691 (DESIGN_cadence.md §2.2) — summary/resolvedAt: chat.js wrote both but this closed list dropped them, so resolvedAt was NEVER set → the `!conv.resolvedAt` guard stayed permanently true and every VITALS_CHANGED re-patched + re-logged every closed incident forever (the vitals sidecar §6.3 clones this path)
