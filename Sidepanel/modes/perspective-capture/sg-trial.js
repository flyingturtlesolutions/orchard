...
export function createSgTrial({ getGroundId, getTabId, getIntent, rerender, afterAccept } = {}) {
  ...
  async function onRun() {
    ...
    rerender?.();
    let res;
    try { res = await new Promise((r) => chrome.runtime.sendMessage({ type: 'RUN_SG_TRIAL', payload: { tabId: getTabId?.(), groundId: getGroundId?.(), intent } }, r)); }
  ...
  return {
    ...
    renderRunButton() {
      if (plan && plan.runnable && !trialInFlight) return `<button class="btn-secondary tiny" data-perspective-action="run-sg-trial" type="button" title="Run the substrate-grounded plan on this page — fills the fields; the irreversible submit is deferred.">▶ Run on page</button>`;
