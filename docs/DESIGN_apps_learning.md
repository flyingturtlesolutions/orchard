...
- **10.2 Distill up. _(BUILT v2.74.1261 — the explicit `distill` / `teach preset` command; an auto-surface trigger is the deferred follow-on.)_** The `distill` command lists this app's `confirmed` learned deltas (`distillCandidates` — facts barred, preset-sourced rules barred) → per rule, an LLM **abstraction** step (`AnthropicService.abstractRuleForPreset` → the `ABSTRACT_RULE` handler) strips specifics ("verify the refund window" kept; "Acme's window is 30 days" dropped, or declines → nothing rises) → the user **confirms** the generalized rule → it's written to the preset store (`recordGoalItem('preset:<presetId>', …)`, provenance `distilled-up`) **and** the instance copy is canonized (`promoteGoalItem(…, {confirmedByHuman:true})` — marks it done, won't re-offer). **HITL-gated**: crossing into the shared preset carries the §7 human gate — the confirm IS that gate (it operates on `confirmed`, canonizing as it rises, since there's no separate canonize UI). Beliefs (facts) NEVER rise. Pure prep/abstract/validate in `Core/presetMemory.js` + `Core/presetAbstractPrompt.js` (tested); the command + LLM call are live-only.
...
- ✅ **WF-1 — the structured workflow flywheel, slice 1 (v2.74.1252).** A clean AUTONOMOUS compound (a connector read / fan-out chain — what the Ground-composite saver can't hold, no Ground) offers **"💾 Remember this workflow"** at the chain's end → bank `{ask, subAsks}` per-instance (`Services/Storage/WorkflowStore.js`). RECALL: at `_tryInterpret`'s top, `workflowMatch` (`Core/workflowMemory.js`, pure+tested — overlap-coefficient, so a short re-ask "get tickets" matches a long saved workflow; precision-biased) finds the saved workflow. SUGGEST-and-confirm: a strong match → a "🔁 …Run it?" card (never silent — autonomous + side-effectful). REPLAY: re-dispatch the subAsks through the SAME chain runner (`_orchRunChain`), so the inner map/fan-out/write gates still apply; bumps a run-count (corroboration + the match tie-break).
- ✅ **WF-2 (v2.74.1254).** NAMING — the bank offer takes an optional alias ("standup"); a name-exact match beats ask-overlap in `workflowMatch`, so typing the alias runs the workflow. MANAGE — the `workflows` command lists this app's saved workflows (name·ask·steps·runs) with ▶ Run / 🗑 Delete (transient menu). DISMISSAL SUPPRESSION — "No, interpret it" bumps a dismissed-count; a never-run, twice-dismissed match stops suggesting (`workflowMatch` filter). **Still deferred:** in-place rename (delete + re-save), a positive run-count suggestion threshold, and `<LEARNED>`-block awareness (kept out so the IL can't "act" on a non-tool ref).
- ✅ **WF-3 — semantic recall (v2.74.1260).** Recall is now a CASCADE: lexical `workflowMatch` (free, deterministic) FIRST; on a miss, escalate to an LLM matcher (`AnthropicService.matchWorkflowSemantic` → the `MATCH_WORKFLOW` handler, Haiku-tier) so a PARAPHRASE ("pull my tickets and dig into each") still finds the saved "get my open tickets, research each". **Cost-bounded** — the LLM fires ONLY on a near-miss (`workflowSharesVocab`: the ask shares ≥1 meaningful token with a saved workflow), so the common unrelated ask pays nothing, and only on an app instance with saved workflows. **Safe** — the model returns one id, VALIDATED against the live non-suppressed set (`resolveWorkflowMatch` guards a hallucinated / stale / suppressed id), and the suggestion stays the same suggest-and-confirm card (never silent replay). Pure build+parse (`Core/workflowMatchPrompt.js`) + prep/validate/gate (`Core/workflowMemory.js`) — all tested; the live LLM call + the render need an eyeball.
