...
**Files:** new module + EE, WorkflowExecutor, tests. **Size:** M. **Status (.943):** DONE — landed as `Core/bindingResolve.js` (chrome-free): coerceTagged/resolveBinding/resolveBindings/scopeLookup; EE's fragment/sieve/NAVIGATE sites adopt with policies explicit; the v2.50 `field` path now in the one resolver. DELIBERATE deviation: WorkflowExecutor.resolveBinding NOT folded — its 3-source lookup chain (paramValues->workflowScope->iterStack) is a different shape; documented as a pairing comment at WE.
...
**Files:** new module + 6 call sites. **Size:** M. **Status (.944):** PARTIAL BY DESIGN — `Services/TabUtils.waitForTabComplete` ({ok} envelope, never rejects, already-complete race handled) adopted by the three RESOLVE-shaped sites (WorkflowExecutor, PageProbe, DiscoveryService — which GAINS the already-complete poll; its reject contract preserved at the call site). The EE + TW variants (grace-period sleeps + reject contracts woven into navigation) are DEFERRED to CR-X2 where they move under live verification.
...
**Files:** Services/AnthropicService.js. **Size:** M. **Depends:** CR-E5 (lands the retry/timeout these calls then inherit). **Status (.945):** DONE — shape deviation: rather than widening #call's signature, the transport core was extracted as `#post` (timeout/cancel/one-retry; #call now consumes it) and a tool-forced sibling `#callTool` added (POST + #audit + usage; tool_use extraction stays with callers). The three frontier methods migrated (audit/cost/cancel coverage gained; hardcoded model strings -> MODEL/MODEL_OBSERVATION_FRONTIER); their early no-key guards kept via hasLlm() so the no-key envelopes stay byte-identical (recovery's legacy `items: []`). #extractJson folded into #firstJsonObject{bestEffort}. Accept's live vision-read retest still owed to the next `gl` session.
...
**Files:** chat.js, Core/walkLedger.js (new), Core/orchRun.js, Core/orchVisual.js, tests. **Size:** L (can split ledger/renderer vs step-runner). **Depends:** CR-S3 (endWalk consumes the ledger). **Status (.946):** DONE — Core/walkLedger.js (walkRecap/walkBoundary/walkEndLines; stop-at-end + preSkipped pinned by 7 tests); scanPlan in orchRun adopted by all FOUR chat walkers (the two driver scans, the bindings collector, AND the find-first untaught-driver walk); `_runResolvedStep` shared by _orchRunPlan/_orchRunChain (read/REPLAY/alias/settle core; messages/offer-record/promotion stay caller-side); renderPlanLines in orchVisual consumed by both confirm cards (the comprehended card upgrades to the superset renderer: wait labels, collect suffix, gate-cond clause fallback). Visual smoke of the confirm cards owed to the next live session.
...
**Files:** Services/ExecutionEngine.js, Services/StrategyTree.js, new registry. **Size:** L (X1a M, X1b S, X1c M). **Status (.947-.948):** X1a+b+c DONE — Services/Engine/nodeRegistry.js (NODE_TYPES / describeNode / childLists; pure, 5 tests; npm-test glob gains Services/Engine/*.test.js). EE: #NODE_EXEC wiring map — DELIBERATE shape deviation: execute handlers stay engine-side as arrows over the private executors (JS #-private is unreferenceable cross-module), registry owns the metadata, a startup alarm logs wiring drift. #executeNodes unified (body mode = fixedIndex + iterationLabel + iteration; opts forwarded uniformly — the old chains' per-type forwarding differences live in the executors' own destructuring); #executeBodyWithIterationLabel DELETED (7 call sites repointed); #describeNodeForPause DELETED (registry describeNode); #executeSingleNode kept as a 4-line registry dispatch (single-node trigger semantics: null = hard failure, no after-node yield). Unknown-type policy unified: normalize=null-with-WARN (warn added), validate=ERROR (new tail branch; was silent fall-through), execute=FAIL-LOUD (was warn+skip; safe — every executed body passes normalize first, which drops unknowns to null). StrategyTree walkNodes + computeIterationScopes descend via childLists (foreach stays the one scope-INTRODUCING special case). Accept grep: ONE strategy node.type switch remains in Services (the registry's childLists); SchemaValidator's two switches are the UPPERCASE LLM-procedure IR (FOR_EACH/DETECT/SEQUENCE) — a different type system, out of X1 scope. Live smoke (a foreach strategy + a paused run's node labels) owed to the next session.
...
**Files:** Services/ExecutionEngine.js → two new modules. **Size:** L. **Depends:** CR-X1a. **Status (.949):** DONE — SieveExecutor.js (1,721 lines: node + template/transform/frontier-compose tiers + 6 helpers; substituteAnalysisParams PUBLIC — shared by obs AND the fragment node's postcondition substitution, a third caller the review missed) + ObservationExecutor.js (1,351 lines: node + vision frontier + blob/tab-url helpers; params resolve via the Core resolver with the fragment policy restated per D4). Moved as CLASSES with byte-identical bodies (indentation + prompt template literals untouched); member refs retargeted; Logger tags stay 'ExecutionEngine' for trace/`gl` continuity. NEW: 5 isAborted checks (sieve node + each tier boundary + the per-operation loop — the span had zero); frontier-compose needs no per-record check (sync prep loops around ONE LLM call). The direct-sieve entry's minimal ctx gained isAborted: () => false (the new checks would have TypeErrored there). _condList moved to Core/postcondition.js as exported condList (obs used it cross-module; +2 tests). EE: 5,174 → 2,170 lines (-3,004; accept was ≥2,000); six orphaned imports pruned. Live smoke (one cache observation + one sieve strategy) owed to the next session.
...
**Files:** background.js, background/handlers/*. **Size:** L (a/b S, then M per domain). **Status (.950):** FOUNDATIONS DONE — (a) assertCtx throwing at wiring time + REQUIRED_CTX_KEYS (the 18-key contract listed ONCE; the JSDoc had drifted to 13) + createSgMessageHandlers asserts on entry; (b) _invokeSgHandler(type, payload) adopted by all FOUR hand-rolled bridges (the review counted three: auto-monitor START/STOP + the workflow executor's runObservation AND runCapability handoffs) — the auto-monitor wrap previously HUNG (and leaked its busy flag) if a handler rejected before responding; (c) RUN_PERSPECTIVE_TRIAL migrated into the registry beside RUN_SG_TRIAL (ctx already carried everything it needs; busy-marking shared via runTrialBundle's internal mark). Legacy switch: 170 → 169 cases. X3a (.951): DONE — background/handlers/explore.js (448 lines): EXPLORE_PAGE_STRUCTURE moved whole with its in-flight set; 8-key asserted ctx (readLocaleCache/writeLocaleCache/normalizeUrl/readSiteMap/mergeSiteMapForGround/readGroundChrome/deriveGroundChrome/appendOutcomes); handler RETURNS its promise chain so the dispatch safety net applies; domain maps now MERGE into the one registry object. NOTE: ensureGroundForUrl already lived in sg.js (the spec text predated G1-1's landing) and the locale-cache helpers were already GroundAssetStore shims — the case + state were the real move. Switch: 169 → 168 cases. X3b (.952): DONE — background/handlers/discovery.js (280 lines): START_DISCOVERY + ABORT_DISCOVERY moved whole with the per-ground abort-flag map and the Cloudflare-tolerant in-tab sitemap fetcher (which adopts Services/TabUtils.waitForTabComplete — background's hand-rolled wait was a SEVENTH D5 copy, kept behind for its 2 other callers); 2-key asserted ctx; DiscoveryService/SitemapService imports pruned from background. X3c (.953): DONE — background/handlers/workflowDebug.js (604 lines): the 12-label/10-body workflow cluster (SAVE/DELETE/INVOKE/CANCEL/PAUSE/RESUME/STEP/STEP_OVER/SET+CLEAR+TOGGLE_BREAKPOINT/GET_BREAKPOINTS) + cancellation set + debug-state map + runSeq + the two sidepanel broadcasts. Case bodies BYTE-IDENTICAL inside a domain-local dispatcher (the tri-label breakpoint case branches on msg.type, which the registry does not pass — a msg shim preserves it); ctx = { invokeSgHandler (the X3b bridge), ensureContentScript }; the keep-alive set stays global (CapabilityAPI lifecycle subscriber). executeWorkflow import pruned. Legacy switch across X3: 170 → 154 cases; background.js shed ~1,160 net lines this sub-arc. X3 COMPLETE except future domains adopt the same pattern as they come up.
...
**Files:** ContentScripts/contentScript.js, studio.js. **Size:** L (split into two slices). **Status (.954):** DONE — X4a: the onMessage listener's 2,066-line switch over 65 message types (46 block cases + 17 braceless + 2 one-liners; the other ~60 'case' hits were NESTED action switches) converted to MESSAGE_HANDLERS map + a six-line dispatcher. Handler bodies byte-identical: each opens with the listener's `const { type, payload }` destructure, block cases keep their braces (block scoping preserved), return values keep the channel-open contract; unknown type → false (the old default). X4b: the STORAGE_CHANGED microtask flag (same-tick-only coalescing) replaced with a 150ms TRAILING debounce — a cascading delete's N macrotask messages now render once; direct refreshGroundList callers keep their awaited single-flight semantics. _refreshGroundListImpl (1,261 lines) reduced to fetch + loop; the whole per-ground card builder extracted as _renderGroundCard(ground, { list, localeMap }) with a byte-identical body. DELIBERATE deferral: the per-SECTION split within the card builder waits for a studio live-smoke harness (pairs with the CR-D8-deferred shared.js moves). Live smoke owed: picker/explore/monitor + a cascading delete.
...
**Status (.955):** DONE — ExtractionEngine.js + runtimeState.js DELETED (zero importers; git has them). locale.js: 8 dead query exports DELETED (featuresForRole/knownSelectors/scrollTargetFor/collections/disclosureFor/goals/edgesFrom/edgesTo), featuresByKind demoted to module-private; the LIVE trio (localeEdges/edgesByKind/pathToGoal) kept. GroundMatcher: decision DEFERRED per this slice's own text (canonicalizeUrl is reimplemented in contentScript — adopt-as-mirror vs delete waits for the next ground-matching slice). Frontier tier button disabled with an explanatory tooltip (storage tolerance kept).
...
**Status (.955):** DONE — 117 lines stripped across ~117 files (79 jsdoc-style ` * @version` + 38 comment-style `// @version`). Zero remain in source.
...
**Status (.955):** DONE — Core/formCoverage.test.js (8 tests: toSlug/stripAsterisk/selectorForField incl. volatile-id rejection/necessity/slugMatch/matchRolesToFields/assessIntentCoverage both shapes — pinning the PB-10 gate's contract ahead of its wiring) + Core/outcomes.test.js (3 tests; hashId's exact djb2/base36 values PINNED — a silent change would orphan persisted intentContext fingerprints). intentShape landed with D3 (21 tests); Logger with T1; GroundMatcher rides the H1 deferral.
...
**Status (.955):** DONE — 41 raw id attribute interpolations escAttr-wrapped (37x ground.id + 4x p.id; ids arrive via cloud sync/import); dead GET_GROUND_READINESS branch dropped from _DECISION_RE. Word-prefix anchors: SKIPPED (the 'consider' item — current prefixes haven't false-matched in any trace).
...
**Status (.955):** DONE — recorder uid gains a per-frame random salt (cross-frame seq+Date.now() collisions); monitor type-debounce is per-ELEMENT (WeakMap timers + a pending Set so detach can clear — fast field-to-field typing no longer drops the first field's event); top-frame-only monitor scope documented at _imAttach (deliberate privacy/noise posture); the re-injection guard still throws (a classic script cannot top-level return) but a one-shot window error handler preventDefault()s exactly that error, so the console stays clean.
