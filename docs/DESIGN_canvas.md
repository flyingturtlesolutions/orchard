...
- **The external doc must SURFACE (GD-4c, v2.74.1325).** A gdoc render opens the Doc in a browser tab on first compose and focuses it on later ones (`focus:false` = silent repaint for background cadence). Live lesson (.1324): a successful paint into a Doc nobody has open reads as "nothing happened" — "no extension canvas tab opens" never meant "no surface". The open Doc then live-updates from the API's `batchUpdate` (Docs realtime) while the user steers from the panel.
...
| **GD-7a** | capability profiles per backend + honest degradation in the lowerings (image→link, chart→table; degrade NAMED in the panel line) | ✅ pure |
| **GD-7b** | rich blocks ref-resolved: gdoc `insertInlineImage` for trusted-map srcs (BUILT v1333 — allowlist + uri deep-check; public CDNs direct, auth-gated attachments still need the signed-URL hop) · tab renders images/video tiles (v1330) | ✅ built; ⛔ needs Lambda redeploy |
| **GD-7h** | deliverable vocabulary WIDENED (BUILT v1335): headings + inline `![alt](kb:...)` ref-images are deliverable (html_body/Gmail render both — the old subset encoded a wrong surface assumption); one parser change feeds all three lowerings (Docs insertInlineImage mid-flow / HTML `<img>` / text `[image: alt]` with the loss named); minted-URL inline images collapse to alt text at the compose entry; operator guidance never renders into the artifact | ✅ built; rides GD-7b's redeploy |
| **GD-7f** | `==highlight==` in the markdown subset → Docs `backgroundColor` · HTML `<mark>` · text degrade — parity across all three lowerings; unicode glyphs (→ ⚠ ✓ ①) already free | next |
| **GD-7g** | ANNOTATED screenshots (the non-generic-guide lever): OffscreenCanvas compositing in the SW — arrow/box/circle specs baked INTO the image before insertion (Docs has NO programmatic shapes — the one true API limit; annotation must live in the bitmap) | after 7b live |
| **GD-7c** | user-preferred backend per instance: AS-2 setup choice + `canvas: use <backend>` re-bind, persisted on the configured def (AP-0) | ⚠ live |
| **GD-7d** | `notion` backend adapter (ensureContainer = create-page-once; render = replace-children) — the broker leg pre-exists (MCP_SERVERS + MP-2c discovery) | ⚠ live |
| **GD-7e** | compose-with-sources: a read's banked result (article text + enumerated `kb:` media menu) rides into `composeCanvas` as a fenced SOURCES block; `video` kind + per-surface degradation (link in gdoc/delivery) | ✅ pure core (prompt+lowerings) · ⚠ live (the read→bank hop) |

**8.7 Surface-agnostic canvas — user-preferred backends + rich blocks (designed 2026-07-02, post GD-live).**
The spec/backend split already IS the surface-agnostic architecture (one CanvasSpec = truth; a backend = one normalized `presentation.backend` + an adapter). What §8.7 adds:
- **The adapter contract, named:** a backend = `{ ensureContainer(anchor) → containerId (create-once, cached, vanished→recreate-once), render(containerId, spec) (replace-body from the spec, never read back) }` — exactly the shape `_renderGdoc` already implements. New surfaces implement the pair; everything upstream (compose, revision turns, the store) is untouched.
- **Capability profiles + honest degradation:** each backend declares which block kinds render NATIVELY vs DEGRADE (image → link, chart → table/text, embed → link-card) vs drop. One spec, per-surface lowerings; a degrade is NAMED in the panel line (§8.3 fidelity — never a silent downgrade). The compose prompt stays backend-blind; the renderer degrades.
- **User-preferred backend, per instance:** `presentation.backend` stops being catalog-fixed — the catalog value is the DEFAULT; setup (AS-2) offers the choice ("compose in: Orchard tab / Google Doc / …") and a runtime `canvas: use <backend>` re-binds; persisted on the CONFIGURED def (AP-0), so each app instance keeps its owner's preference.
- **Rich blocks (the support case — screenshots in a draft): refs, not URLs.** The display vocabulary already carries `image` (CA-1, src-sanitized; the tab renders it) — the gaps are sourcing + coverage. Rule: the model may only REFERENCE trusted artifacts — `attachment:<id>` (a connector-read ticket attachment) or `capture:<id>` (an Orchard page screenshot) — never mint remote URLs (an LLM-authored image URL is an exfiltration beacon: data rides the query string the moment the surface fetches it; normalizeCanvasSpec drops raw external URLs). Adapters resolve refs from the trusted store: tab = data URI · gdoc = `insertInlineImage` (needs a fetchable URI — a signed short-lived cloud URL, the Lambda's job) · delivery HTML = attachment/cid. "Embeds" are TYPED (link-card, video-link, image) rendered by OUR components — never raw iframes.
- **Backend candidates by profile:** `tab` (richest — ours; images/effects free) · `gdoc` (prose + images-with-caveat; durable/shareable) · `gsheet` (data; GD-6) · **`notion`** (images/embeds/databases native; already in MCP_SERVERS with MP-2c live tools/list — the broker leg is pre-built, the adapter is the only new work).

**8.7.1 The KB-remix workflow (the motivating rich-block case, 2026-07-02).** A support app *grabs a KB article* (Ride/Broker read — Zendesk Guide, Notion, intranet) → *composes a troubleshooting guide TAILORED to this user* on the canvas, carrying the article's images/videos → the user reviews/modifies from the panel (GD-4, exists) → the app *sends* (GD-5, CA-8-gated, exists-as-design). It forces exactly two new pieces:
- **Source banking + the `kb:` ref class:** the read RESULT is banked as a source artifact with its media ENUMERATED (`kb:<articleId>#img2` → the KB CDN URL). The composer picks from that menu; refs-not-URLs holds — and it is precisely what contains a POISONED article ("include this image http://evil…" cannot happen: the URL isn't in the catalog, normalize drops minted ones). KB media is usually on the org's PUBLIC help-center CDN → directly fetchable by Google → the gdoc `insertInlineImage` hop needs NO signed-URL Lambda for this class — the KB case is the EASIEST first image slice.
- **Compose-with-sources (GD-7e):** `composeCanvas` gains a fenced `SOURCES` block (the fetched article: text + the media-id menu; DATA, never instructions) — the §8.5 "get context → compose" joint, plumbed. `video` joins the vocabulary with clean degradation: player/link-card on tab · LINK in a Doc (Docs cannot embed video) · link in delivery HTML — every downgrade named (§8.3).
