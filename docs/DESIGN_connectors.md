...
- **Google, concretely (REVISED 2026-07-01, live-proven):** Google's Workspace MCP servers exist (`calendarmcp.googleapis.com` etc.) but are **Developer-Preview-gated, and the preview program rejects consumer @gmail.com accounts** — the live `PERMISSION_DENIED` proved a server *existing* ≠ *eligible* (coverage research needs an ELIGIBILITY column). So Google serves via the **GA REST channel** (`lambda/api/googleRest.cjs`, v1318): same tool ids, same vaulted token + scopes, per-server `CONNECTOR_CHANNEL` map in `handleConnectorInvoke` — flip a server back to `'mcp'` when Google ships GA (or for Workspace-account users). The extension never knows which channel served it. *(Client-side alt: `chrome.identity.getAuthToken` + direct googleapis from the SW — no backend at all; Google-only, browser-bound, not headless.)*
...
**Status (v2.74.1305–1309):** CX-5a built — `Core/brokerCatalog.js` (curated Google Calendar + Gmail → `impl:'oauth'` legs via `mcpToolToLeg`; closes the leg-availability loop). CX-5b built both sides — `Core/brokerInvoke.js` fail-closed write gate + `INVOKE_CONNECTOR` handler + `CloudClient.invokeConnector`; Lambda `/v1/connectors/{invoke,tools,link/*}` with server-side re-checks + honest degradation. MP-1 `Core/mcpClient.js` (canonical pure MCP core) + MP-2a `Core/mcpServers.js` (curated id→endpoint registry; Zendesk deliberately absent). MP-2b — the Lambda SPEAKS MCP: `lambda/api/mcp.cjs` (initialize → initialized → tools/call, shared deadline, anti-SSRF server-side endpoint map) + generic OAuth2 refresh-token exchange in `handleConnectorInvoke`; **the CJS file is a deliberate mirror (no build step to share ESM into the Lambda asset) kept honest by a PARITY test** (`Core/mcpLambdaTransport.test.js` — envelopes vs mcpClient, endpoints vs mcpServers; drift turns `npm test` red).

**MP-3 BUILT (v2.74.1310), per the pinned contract:** the OAuth dance runs CLIENT-side via `chrome.identity.launchWebAuthFlow` (redirect `https://<ext-id>.chromiumapp.org/{provider}`, PKCE); the extension POSTs `{code, redirectUri, codeVerifier}` to the JWT-authed `POST /connectors/link/{provider}`; the Lambda exchanges code→refresh token (client secret stays server-side, env) and vaults it at `CONNLINK#{provider}`. NO unauthenticated callback route, NO CDK change, the secret never touches the client. Pieces: `Core/oauthLink.js` (PKCE S256 + authorize-URL + state-VERIFIED redirect parse) + `mcpServers.providerScopes` (one dance grants every server the provider fronts) + `lambda/api/oauth.cjs` (code-exchange + refresh, injectable fetch, headless-tested; a grant with no refresh token = a failed link) + real `handleConnectorLink`/`handleConnectorUnlink` (vault write; unlink = best-effort revoke + delete) + `LINK_CONNECTOR` handler (client id from `chrome.storage.local connector:oauthClientId:{provider}`; logs `CONNECTOR_LINK ▸`, in `_DECISION_RE`) + `CloudClient.linkConnector`/`unlinkConnector`.

**CX-5c BUILT (v2.74.1311) — the arc is CODE-COMPLETE:** broker legs surface in the interpret palette GATED on linked providers (`brokerLegsForLinked` + the `connector:linkedProviders` cache LINK/UNLINK maintain; an unlinked provider's legs stay out — a selectable-but-dead leg reads as broken); a broker WRITE pick gets the same chat-side HITL confirm bar as a ride write (exact `server · tool` + args preview → `confirmed:true`, with INVOKE_CONNECTOR + the proxy both fail-closed behind it); `link: <provider>` / `unlink: <provider>` chat commands are the user-facing trigger; `UNLINK_CONNECTOR` + `GET_CONNECTOR_STATUS` handlers complete the lifecycle (backend truth refreshes the cache; cloud-unreachable degrades to the cache marked stale).

**What remains is EXTERNAL + DEPLOY only (no code left before them):** ① register the Google OAuth client — redirect URI `https://<ext-id>.chromiumapp.org/google`; the id → `connector:oauthClientId:google` (chrome.storage.local), id+secret → Lambda env `GOOGLE_OAUTH_CLIENT_ID/SECRET`; ② `cdk deploy` (ships index.js + mcp.cjs + oauth.cjs); ③ live eyeball: `link: google` → consent → `CONNLINK#google`, then "list my calendar events" → the first real broker round-trip, then a gated `create_event` write → the thesis closed end-to-end.
...
> **§17.0 Builder trigger (ruled 2026-08-07, lane-663f panel):** the proactive 'learn this site?' offer is a NEW TRIGGER over this pipeline, not a new mechanism: an uncataloged-AND-unlegged host the user repeatedly ASKS about (adopt-on-2nd counter shape — n≥2, offer once, decline sticks; never mere tab presence) mints ONE proposal card on the existing spine; accept deep-links the add-leg wizard into Discovery/Forage → recipeFromHarvest → `pending` behind the §18 arm guard. Predicate is 'no armed legs', not 'not in catalog' (harvested recipes are co-equal). Scarcity governed by the SGV demand class (DESIGN_session_governor.md v1.15). Detection stays local-deterministic (host-suffix match, zero egress; tab hosts NEVER logged — the scrubber does not cover URLs). Leg suggestions stay ride-layer only; PS-3/4 DOM synthesis remains parked.
...
| **Forage** *(new)* | **yes — CLIENT-SIDE nav in the LIVE logged-in tab** | **read-safe `<a href>` clicks** | **the AUTHENTICATED read-API SURFACE** | **rides the live tab (route restored after)** |
...
7. **Rides the LOGGED-IN session via CLIENT-SIDE nav in the LIVE tab** (v2.74.1283 — the corrected mechanism; v2.74.1282's tab-DUPLICATE was wrong). Real apps (the Deako CS tool — a static S3 SPA) hold their auth token **in memory**, so a fresh tab, a duplicate, OR a hard `tabs.update` reload all boot ANONYMOUS — cookies transfer but sessionStorage + in-memory auth don't survive a fresh document. Forage instead injects a driver into the user's **live logged-in tab** that **clicks read-safe in-app `<a href>` links** → the SPA router changes route CLIENT-SIDE (no reload → the in-memory token survives → the app fetches each route AUTHENTICATED) → the §17 tee (injected on the live tab) captures those reads. It clicks ONLY same-origin `<a href>` with a non-destructive/non-write label and a non-auth path (never buttons/divs — those could be actions), restores the start route CLIENT-SIDE (`history.go`, never a hard nav = never logs the user out), and is bounded (MAX_VISITS). `sessionTabId` (the panel's active tab / Discovery's tab) is REQUIRED — it's the only context with the token. The background hard-nav URL-frontier (`Core/readSafe.js` + `Core/forageFrontier.js`) is **parked** for a future cookie-auth MPA mode (it can't ride in-memory auth).
...
## 20. CX-8 — writes-as-demos: dual-capture (drive + ride) on one demonstration (designed 2026-07-08)

**The problem (why writes are the odd one out).** §17/§19 harvest READS autonomously — Forage fires read-safe affordances and the tee banks the endpoints, zero user action. Writes can't be autonomous: firing a write has side effects, so the user must demonstrate the ones worth teaching (§17, build-order item 5, the pending slot). But the *live* Shopify port shipped a worse thing than "demonstrate once" — a bespoke **manual hash-capture ritual** (CX-7b/T4: "perform a `CustomerCreate` by hand so the sniffer banks the persisted-op sha"). That step fits neither acquisition model the user already understands — Drive (*demonstrate*: "show me" → demo → bake) or Ride (*discover → forge*) — it is an **implementation detail (the persisted-op hash) leaking out as a user gesture.** CX-8 dissolves it: **one demonstration captures BOTH a drive and a ride.**

### 20.1 Why the ride *alone* is fragile — the persisted-op hash (the motivating crux)
Shopify's admin GraphQL **mutations** use **persisted operations / trusted-document allowlisting**: the client doesn't send the mutation text, it sends a **sha256 of the operation document** at `POST /api/operations/<sha>/<OpName>/shopify/<handle>` + `{operationName, variables}`. The server runs ONLY operations whose hash is in its registry.
- **What the hash IS:** a content-addressed *identifier of one GraphQL operation document* (a git-SHA of the mutation's exact text/shape). NOT a token, NOT per-request, NOT auth. Auth is still cookies (session) + `x-csrf-token` (the CSRF token we sniff — that is the "this request really came from the logged-in page" gate). The hash is a *different* gate: **authorization to a specific allowlisted operation.**
- **Why it ROTATES:** the hash = sha of the document, so any admin deploy that changes that mutation's document (a new selection field, a fragment tweak) mints a new hash; the server's registry only knows the *current* build's hashes → an old hash 404/406s (`HASH_STALE`).
- **Why UNIQUE to Shopify writes:** (a) *vs Zendesk* — Zendesk is plain REST (`PUT /api/v2/tickets/{id}.json`); any well-formed request the API accepts runs, so a curated recipe is a stable public contract forever. Shopify gates mutations behind a private, rotating allowlist. (b) *vs Shopify reads* — our reads use the **ad-hoc** endpoint `/api/shopify/{handle}` with the FULL `{query}` (Shopify permits ad-hoc *queries*); only **mutations** are allowlist-restricted, because a write's blast radius justifies limiting a session to Shopify's own vetted operation set (a CSRF-bypassing attacker still can't craft a novel destructive mutation). So the hash problem is *write-class × persisted-op-API* — most REST connectors never hit it.
- **The consequence, and the fix:** a hash can't be *curated* (private + rotates) — it must be **observed from the live client.** Replaying the ride ourselves requires knowing the current hash → capture + rotation fragility. **Driving the DOM (clicking Save) makes Shopify's OWN client construct the request with the hash IT currently holds → we never touch the hash → the drive is immune to rotation.** ∴ **the drive is the durable floor; the ride is the speed optimization layered on the same demonstration.**

### 20.2 The thesis — one demo, two artifacts
When the user demonstrates a write (navigate → fill form → Save), the app's SPA fires its own request under the hood. A single demonstration therefore feeds **both** lanes at once:
| lane | captured from the demo | replay | property |
|---|---|---|---|
| **Drive** (DOM) | the OBS trace: navigate + TYPE + CLICK Save, landmark-backed | drives a visible tab, the SPA supplies the hash | **durable** — self-healing selectors, immune to hash rotation; but needs a tab (not headless) |
| **Ride** (network) | the §17 tee sees the `POST /api/operations/<sha>/…` → `recipeFromHarvest` forges the persisted-op recipe (`op_sha` from the bank) | replays the POST headless | **fast + headless** (the sweep wants this); goes `HASH_STALE` on deploy |
The demonstration IS the ride-*discovery* (§17's forge, applied to writes). The word "hash" never reaches the user; the standalone T4 ritual is retired (the `ops` viewer becomes a diagnostic, not a required step).

### 20.3 Dispatch — prefer ride, fall back to drive (the degrade-correctly win)
For a write class that has both artifacts, dispatch **prefers the ride** (fast/headless — required for the *unattended sweep*, which can't drive a visible tab) and **falls back to the drive** (DOM) when the ride is absent or returns `HASH_STALE`. So a rotated hash *silently degrades to the DOM path* instead of dead-ending on "re-capture the hash." Interactive teach with no ride yet → the drive runs immediately (no capture wait). This is the write-class analogue of §15's per-step lattice: the router picks the cheapest *working* execution per call.

### 20.4 The acquisition flow (the UX — identical to every other capability)
`create a customer` → interpret finds no capability → **"I don't have that yet — show me"** → user demonstrates it once in the admin → dual-capture bakes the drive (always) + forges the ride (when a persisted-op fired). Next time it just works. The teach flow states plainly **"this demonstration will actually create a record"** (a write demo has a real effect — unlike a read Forage) and lands the result **`pending` + `gated`** in the §18 per-Ground Ride/Drive surface for human accept before it is armable.

### 20.5 Safety (unchanged floors, restated for the write demo)
- **Method IS the safety class (§9)** — a demoed/forged write is `gated`, **never** auto-armed regardless of origin; `confirmed:true` at both belts (§9) is untouched.
- **Money / inventory NEVER (§14)** — a demonstration of checkout/refund/pay is refused; only the ALLOWED classes (customer create/edit, draft order) are teachable, matching the ride-legs policy.
- **The persisted-op `op_sha` is TRUSTED-fill only** — from the bank, never model text (CX-7b, already enforced).
- **Verify identity, not status (§14)** — a write demo on a logged-out session is refused before it records anything.
- **Busy-mark exception (Invariant #2)** — a user DEMONSTRATION's clicks ARE the signal: do **not** suppress the OBS recorder, but DO arm the §17 tee alongside it (the same "observe the user" exception the drive flywheel already makes).

### 20.6 Maps onto what exists (mostly wiring)
**Reuse:** OBS-1..4 (demo → fragment → durable capability, built) · the §17 body-blind tee + `start`/`stopHarvestSession` + `_bankHarvested` (built) · `recipeFromHarvest` (built) · the §18 per-Ground Ride/Drive collection + arm guard (built) · the CX-7b persisted-op recipe shape + op-bank (built) · the connector write dispatch + HITL belts (built).
**Net-new (small):**
- **Dual-capture wiring** — arm the §17 tee *during* an OBS **write** demonstration (not just Forage/Discovery crawls) and, on stop, hand any observed `/api/operations/<sha>/…` POST to `recipeFromHarvest` → bank a persisted-op ride recipe (`provenance: demonstrated`, `pending`).
- **Prefer-ride-fallback-drive dispatch** — a write-class cascade: ride (if armable + hash fresh) → drive (DOM) on absence/`HASH_STALE`. `Core`-pure selection + the live glue.
- **The "teach this write" entry** — surface the OBS write-demo from the "I don't have that" branch (interpret's teach affordance already exists for drives; extend it to offer a write teach when the target is a connected ride origin).

### 20.7 Build order (pure-first)
1. `Core/writeDispatch.js` *(pure + tests)* — the prefer-ride-fallback-drive selection over a Ground's `{ride, drive}` artifacts for a given write intent; `HASH_STALE`/absent → drive. The tested contract before any glue.
2. **Dual-capture wiring** *(live)* — arm the tee inside the OBS write-demo lifecycle; on stop, forge + bank the persisted-op ride beside the OBS-baked drive. Emit `TEACH_WRITE ▸` (→ `_DECISION_RE`, Invariant #1).
3. **Dispatch glue** *(live)* — the write path consults `writeDispatch`; a ride `HASH_STALE` (the CX-7b 404/406 → `op-hash-stale`) transparently re-routes to the drive instead of surfacing the re-capture hint.
4. **Retire T4 as a ritual** — the manual "do it by hand to bank the hash" prompt is removed from the write path; `ops` stays as a Studio/panel diagnostic.

**Open decision (pin before slice 2):** does the write teach demonstrate in the *user's live admin tab* (real effect, real hash — but creates a real record every teach) or a *scratch/test* context? **Lean:** the live tab with an explicit "this creates a real record" confirm — a write's whole point is the real request+hash, and a test record is cheap to delete; a sandbox would capture a hash that may differ from prod. Revisit if a target app has no safe throwaway write.

### 20.8 The self-heal loop — and its one gap (the unattended sweep)
A stale ride hash **triggers** the drive fallback (20.3), and the drive **refreshes** the hash as a byproduct — **the fallback path IS the re-capture path:**
1. Ride replays `<stale_sha>` → 404/406 (`HASH_STALE`). **This is a PRE-EXECUTION rejection** — the server doesn't recognize the operation, so it runs NOTHING: no record created, no partial write. ∴ the drive that follows is the **first and only** create — **never a duplicate.** (A stale hash fails *clean*; only a *post*-execution failure would risk a double-write, and this isn't one — the safety property the whole loop rests on.)
2. Dispatch falls back to the DOM drive → the **Save click makes the app's OWN client** fire `POST /api/operations/<FRESH_sha>/…` with the hash the currently-loaded build holds.
3. The §17 tee (armed for the drive) observes `<FRESH_sha>` → `_persistSniffedOps` overwrites the stale entry → the ride is **re-primed** for next time.
One `HASH_STALE` costs exactly one slower (DOM) write and self-heals the fast path — no hand re-capture, ever.

**Preconditions:** (a) a **drive artifact must exist** — CX-8 dual-capture guarantees it (one demo bakes both lanes); a *ride-only* bank (the retired T4 path) has nothing to fall back to and dead-ends. (b) the **tee is armed during the drive** (wire it into the drive-fallback lifecycle, exactly like the demo).

**The gap — the unattended sweep can't self-heal *alone*.** The drive needs a driveable tab, but the headless sweep must never drive/steal a visible tab (H-1a). So a sweep write that hits `HASH_STALE` **cannot** fall back to the drive in-flight — it **parks** the write as a pending proposal with an honest note. The re-prime then happens at the **human's approval click**: approving the parked write runs the drive (interactive, drivable tab) → completes the write AND re-primes the hash → subsequent sweep writes use the fresh ride headless again. Net: **a stale hash demotes one write from autonomous to human-gated, and that one human gesture restores autonomy** — graceful degradation, not breakage. (A background-tab DOM drive for the sweep is *possible* but rejected: Chrome throttles/freezes background tabs — the v1380 discard/freeze problem — and a headless DOM *write* is riskier than the park-then-approve flow.)

### 20.9 The write-forge needs the request BODY — the body-blind tee isn't enough (refinement, 2026-07-08)
Forging the RIDE from a demo is NOT symmetric with §17's read-harvest. A **read** is a GET — its recipe is fully defined by `{method, url}`, which the §17 **body-blind** tee (records only `{method, url, status}`, never a body — the privacy invariant) captures completely. A **write** is a persisted-op POST whose recipe also needs the **`variables` template** (`customerInput: { firstName: '{first_name}', … }`) — and that lives in the request **BODY**, which the body-blind tee deliberately does NOT read. So a body-blind observation yields the op name + sha + handle (the endpoint) but **not** the recipe's parameter shape → an incomplete ride.
**Resolution:** the write demo needs a **body-capturing tee variant, scoped to the ONE demonstrated write, consent-explicit.** Justified because it's the user's own deliberate teach (not a passive crawl), it captures exactly one request (not a surface), and the body IS the thing being taught (the variable schema). Kept OUT of the autonomous read-harvest path (Forage/Discovery stay body-blind — the invariant holds there). The captured `{url → op_sha/name/handle, body → variables template}` feeds the forge (`Core/forgeWriteRecipe` — pure, unbuilt) → a CX-7b-shaped persisted-op recipe (`{op_sha}` endpoint, `persistedOp`, the variables template, method POST, `gated`). The `{param}` generalization of the variables comes from the demo's DOM (what the user typed → which field) — the OBS trace already maps typed values to fields, so the drive and the ride share one binding. **This is why dual-capture is one gesture, not two: the DOM demo supplies both the drive steps AND the ride's variable bindings; the body-tee supplies the wire shape.**
