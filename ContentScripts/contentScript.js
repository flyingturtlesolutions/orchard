...
function _classifySelectorTier(selector) {
  if (!selector || typeof selector !== 'string') return 99;
  const s = selector.trim(); if (!s) return 99;
  ...
  const cm = s.match(/\.[A-Za-z_][\w-]*/g) || [];
  const stable = cm.filter((c) => isStableIdent(c.slice(1)));
  if (stable.length === 1) return 4;
  if (stable.length >= 2) return 5;
  return 6;
}
function _mostDurableUnique(cands) {
  const list = (cands || []).filter((s) => typeof s === 'string' && s.trim());
  if (!list.length) return '';
  let best = list[0], bt = _classifySelectorTier(best);
  for (let i = 1; i < list.length; i++) { const t = _classifySelectorTier(list[i]); if (t < bt || (t === bt && list[i].length < best.length)) { best = list[i]; bt = t; } }
  return best;
}
...
function computeUniqueSelector(el) {
  ...
  for (let i = 0; i < MAX_DEPTH; i++) {
    ...
    if (matches && matches.length === 1 && matches[0] === el) {
      if (_classifySelectorTier(path) < 6) return path;   // durable enough — original shortest-stable behavior
    ...
    if (!current || current === document.body || current === document.documentElement) {
      ...
      if (current === document.body) {
        ...
        try {
          ...
          if (final.length === 1 && final[0] === el) { if (_classifySelectorTier(finalPath) < 6) return finalPath; brittleUniques.push(finalPath); }
        } catch { /* give up */ }
      }
      break;
    }
