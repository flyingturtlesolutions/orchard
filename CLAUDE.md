...
## Commands
- **Test (the gate):** `npm test` — node harness over `Core/*.test.js`, `Services/*.test.js`, `Services/Engine/*.test.js`, `Services/Chat/*.test.js`, `Services/Storage/*.test.js`, `bridge/*.test.js`. Must stay green (**4176 passing** at v2.74.2043; the long-stale "808" in this file predated several globs). NOTE `background/handlers/*.js` is deliberately OUTSIDE the glob — cadence/connector/sg changes get `node --check` + `npm run undef` only, never a unit gate, so say so when reporting what a change proved.
...
Plus: `GET_RIDE_RECIPES` (`background/handlers/sg.js`) MERGES curated into a NON-empty Ground — a FULL `mergeRecipes` (v1435): mechanical fields REFRESH on existing records (so a catalog upgrade reaches already-seeded Grounds — add-missing-only left them frozen at seed-time shape, the v1434 live bug), curated `name`/`does` refresh too (catalog-owned text), user state (enabled/reviewState/safetyClass/trust) is preserved, harvested records survive. A hard-deleted curated id re-appears: disable/reject a curated leg, don't delete it.
...
- **End every loop-work reply with the pending human ACTION steps (user direction, 2026-08-05, widened same
  day).** Not just after a build — after ANY loop turn (tick, grade, inbox check, results read): the last thing
  in the reply is a short "Your steps" list — reload (chrome://extensions ↻) plus the exact ask(s) to send,
  verbatim, in order — compiled from the OPEN bus tests' `[human]` checklists, deduplicated (one reload; one
  run of a shared flow covers every test scoped to it). If nothing is owed, say "no human steps pending"
  explicitly. The user drives the live half of every verification; a reply that omits the steps stalls the
  loop, and a no-build turn is exactly when the backlog of unmet `[human]` steps is the loop's bottleneck.
...
- **Re-arm auto-glf when resuming loop work.** The `tools/glf/CRON_PROMPT.md` loop runs on a SESSION-SCOPED cron: it dies when the conversation is cleared, and the loop cannot tell you it stopped — a dead loop and a quiet one look identical until the next `tick.cjs` prints `LOOP GAP`. So at the start of a session that will do build→verify work, re-create it: a recurring 5-minute job whose prompt is *"read `tools/glf/CRON_PROMPT.md` in full and follow it, starting with `node tools/glf/tick.cjs`"*. Point the job at the FILE, never a copy of its text — the file is the source of truth and edits to it must take effect without touching the job. Measured cost of not doing this: 2026-08-04 ran five gaps (42 / 62 / 17 / 20 / 54 min), two of them genuine absence; the 2026-08-03 audit measured a 35.4% duty cycle over a day. The durable fix (an OS-level `schtasks` task running headless `claude -p`) stays UNBUILT on purpose — it needs an unattended agent to hold commit/push rights, and that is the user's call, not a default (`CRON_PROMPT.md` "Still owed", item 1). **Paste shortcuts** (entire first message in a new chat — expansions in `tools/glf/SHORTCUTS.md`): `run glf grader` = grader/dual-role; `run glf builder` = fresh lane, expect lease refused → inbox. (`run auto glf` is a deprecated alias for `run glf grader`.)
