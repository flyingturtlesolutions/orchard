...
const PRESETS = [
  ...
  },
  {
    ...
    baseline: [
      ...
      { kind: 'delta', body: 'WARRANTY SWITCH — the decision procedure. Work each task with these steps, in order, and do not add reasons of your own. STEP 1 — is it ours? This is Deako’s vendor queue and Deako supplies the SWITCHES, so the note need NOT say “Deako”. If the task is about switches OR about lights/power that a switch controls, it is ours. If its subject is something else entirely (outlets, HVAC, plumbing, trim, paint, appliances) it is NOT DEAKO — done. STEP 2 — how many? Take the number the note gives: an explicit count (“4 light switches”→4, “(6) total”→6); a range → its UPPER bound (“4-5”→5); “both”/“the two”/“a pair”→2; several places listed → their SUM (“one for the office, one for the master bath”→2); anything singular or unnumbered but clearly ONE thing (“a light switch sticking”, “the switch is dead”, “1 light flickering”, “please send a new switch”)→1. Read the whole note for the number, and ignore numbers that are not counting switches (lot/unit numbers, dates, dollar amounts, “Gen 2”, “3-way”). STEP 3 — which product? The Simple Rocker Switch (Single-Pole & Multiway) unless the note names a dimmer, a smart plug, or a smart switch — then use that named product. STEP 4 — the answer is REPLACE (count × product) unless one of exactly TWO things is true: (a) you cannot get a number because the note says only an indefinite plural with no count anywhere (“send switches”, “multiple”, “several”), or (b) the note names a dimmer/smart plug/smart switch you cannot find in the catalog. Those two are CONTACT HOMEOWNER. Nothing else is. In particular: a fault with no explicit request is REPLACE, a request with no fault described is REPLACE, a light symptom is REPLACE, an unnamed switch type is REPLACE, and a scheduling note alongside a switch order is REPLACE.' },
