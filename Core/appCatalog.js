...
const PRESETS = [
  ...
  },
  {
    ...
    baseline: [
      ...
      // v2.74.2087 (user parse spec) — the WARRANTY SWITCH PARSER, as standing rules seeded into every instance's
      // LEARNED memory (the right layer: the parser reads <LEARNED>, so any read/branch/draft applies these). Three
      // rules: switch IDENTITY (+ Deako owns switches, a problem = a replacement), COUNT derivation, and the THREE
      // DISPOSITIONS. Resolves the ambiguous-type + distributive/range/no-count + wrong-task parse failures.
      { kind: 'delta', trigger: 'reading a warranty task to decide what Deako owes', body: 'Deako is responsible for the SWITCHES on every warranty task, and sells exactly ONE: any switch phrasing OR any described switch problem — “switch”, “light/wall/3-way/single-pole/rocker switch”, or a switch that is sticking / flickering / dead / buzzing / loose / not working — means REPLACING it with the Simple Rocker Switch (Single-Pole & Multiway). So “light switch sticking” = one Simple Rocker Switch replacement. Dimmers, plugs, and keypads are DIFFERENT Deako products — never map those to the rocker switch.' },
      { kind: 'delta', trigger: 'deciding HOW MANY switches a warranty task needs', body: 'Count rule: an explicit number wins (“4 light switches” → 4; “(6) total” → 6). A single switch problem with no number (“a light switch sticking”, “the switch is dead”) → 1. An enumeration across places (“one for the office, one for the master bath”, “one in each of 3 bedrooms”) → the SUM. If the count is genuinely unclear — a range (“4-5”), or a bare plural with no number (“send switches”, “multiple”, “several”) — do NOT guess a number; that is the CONTACT-HOMEOWNER disposition.' },
      { kind: 'delta', trigger: 'classifying what to DO with a warranty task', body: 'Every warranty task is exactly ONE of three dispositions: (1) REPLACE — a Deako switch matter with a clear count → propose the draft order (count × Simple Rocker Switch); (2) CONTACT HOMEOWNER — a Deako switch matter but the count or specifics are unclear → propose contacting the homeowner for clarity, never draft a guessed order; (3) NOT DEAKO — not about Deako switches (electrical outlets, HVAC, plumbing, trim/paint, or a pure routing/scheduling note) → out of scope, route or skip. Never force an unclear or non-switch task into REPLACE.' },
    ],
  },
  {
