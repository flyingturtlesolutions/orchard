...
const PRESETS = [
  ...
  },
  {
    ...
    baseline: [
      ...
      { kind: 'delta', body: 'WARRANTY SWITCH — the decision procedure. Work each task with these steps, in order, and do not add reasons of your own. STEP 1 — is it ours? This is Deako’s vendor queue and Deako supplies the SWITCHES, so the note need NOT say “Deako”. If the task is about switches OR about lights/power that a switch controls, it is ours. You never need to work out whether the EXISTING switch is a Deako or a standard/builder-grade one — Deako supplies the replacement either way, so “unclear if it’s a Deako switch” is not a question and never a reason to hold the task. If its subject is something else entirely (outlets, HVAC, plumbing, trim, paint, appliances) it is NOT DEAKO — done. STEP 2 — how many? FIRST look for any quantity in the note; if one is there, that is the count and you are done counting. Take the number the note gives: an explicit count (“4 light switches”→4, “(6) total”→6); a range → its UPPER bound (“4-5”→5); “both”/“the two”/“a pair”→2; several places listed → their SUM (“one for the office, one for the master bath”→2); anything singular or unnumbered but clearly ONE thing (“a light switch sticking”, “the switch is dead”, “1 light flickering”, “please send a new switch”)→1. Read the whole note for the number, and ignore numbers that are not counting switches (lot/unit numbers, dates, dollar amounts, “Gen 2”, “3-way”). STEP 3 — which product? The DEFAULT is the Simple Rocker Switch (Single-Pole & Multiway). EVERY ordinary switch wording IS that default — “switch”, “switches”, “light switch”, “wall switch”, “rocker switch”, “single-pole”, “multiway”, “3-way” — and you never need to look one of those up, confirm it, or match it against a catalog name. None of them is EVER a reason to contact: “no catalog information”, “type not specified in catalog” and “not the full product name” are not valid concerns for an ordinary switch. Use a different product ONLY when the note names a dimmer, a smart plug, or a smart switch. STEP 4 — the answer is REPLACE (count × product) unless one of exactly TWO things is true: (a) THERE IS NO NUMBER ANYWHERE for the switches — apply this mechanically: if the note contains ANY quantity for them (a digit like “4” or “(6)”, a written number like “four”, “one”, “two”, a range like “4-5”, or “both”/“a pair”) then you HAVE the count and it is REPLACE, full stop. Only a bare plural with NO quantity at all (“send switches”, “multiple deakos”, “several switches”) is countless. “4 light switches” has the count 4 — never call a numbered note vague, indefinite, or “without specifics”, or (b) the note names a DIMMER, SMART PLUG or SMART SWITCH and you cannot find that specific product — this never applies to an ordinary switch wording, which is always the default product. Those two are CONTACT HOMEOWNER. Nothing else is. In particular: a fault with no explicit request is REPLACE, a request with no fault described is REPLACE, a light symptom is REPLACE, an unnamed switch type is REPLACE, and a scheduling note alongside a switch order is REPLACE.' },
      { kind: 'delta', body: 'WARRANTY SWITCH — the arms. When you classify, sort, group or branch warranty tasks, use EXACTLY these three arms with these labels: “replacement needed”, “contact homeowner”, “not deako”. Never invent other arms (never “requesting replacement / not requesting replacement”, never a two-way split), and never leave a task with no arm — every task lands in one. “Unknown”, “couldn’t tell” and “none” are NOT available here: if you genuinely cannot decide, the answer is CONTACT HOMEOWNER — that arm IS the “I can’t decide” outcome, so use it instead of leaving the task unclassified. Handle one switch line per task; if a task orders different products (2 switches AND 1 dimmer), say so and propose the lines you can rather than dropping one.' },
      { kind: 'delta', body: 'WARRANTY SWITCH — when NOT to act. A note that negates or closes the work is not a replacement: “do NOT replace”, “homeowner declined”, “leave as-is”, “cosmetic only”, “punch item closed”, or a completion record (“already replaced”, “switch has been shipped”, “this is complete”). Treat those as NOT DEAKO for this pass (no draft, no contact) and say why in one short line.' },
    ],
  },
  {
