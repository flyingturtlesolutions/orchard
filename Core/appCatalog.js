...
const PRESETS = [
  ...
  },
  {
    ...
    baseline: [
      ...
      { kind: 'delta', body: 'WARRANTY SWITCH — which product: Deako is responsible for the SWITCHES on every warranty task; a described switch problem (sticking / flickering / dead / buzzing / loose / not working) is a REPLACEMENT — UNLESS the note negates or closes it (“do NOT replace”, “declined”, “already replaced/sent”, “fine now”, “leave as-is”, “cosmetic only”, “punch item closed”): that is NOT a replacement. DEFAULT-ONLY product rule: when the TYPE is NOT specified — a plain “switch”, “light switch”, “wall switch”, or “3-way switch” — use the Simple Rocker Switch (Single-Pole & Multiway); it covers BOTH single-pole AND multiway (3-way), so never downgrade a 3-way to a bare single-pole. Do NOT auto-substitute the rocker when a SPECIFIC type IS named — a dimmer, a smart plug, or a smart switch is a DIFFERENT product: resolve THAT product by its name, never the rocker; if a named product cannot be resolved in the catalog, do NOT fall back to the rocker and do NOT dead-end — that is CONTACT HOMEOWNER (confirm the exact product). CRITICAL: “light switch” / “light switches” IS a switch with an unspecified type — it takes the DEFAULT (Simple Rocker Switch) and is NEVER a reason to ask the homeowner. The word “light” only creates doubt when NO switch word is present at all: a bare “light”, “bulb”, or “fixture” on its own (“1 light flickering”, “the light in the primary bath”) is not necessarily a switch (Deako does not make fixtures/bulbs) → CONTACT HOMEOWNER to confirm switch vs fixture. Never treat an unspecified switch as needing product confirmation: “4 light switches” → REPLACE 4 × Simple Rocker Switch, no contact. (So “light switch sticking” → 1 Simple Rocker Switch; “Need 4 light switches” → 4 Simple Rocker Switches; “Gen 2 smart switch” → the smart switch product; a bare “1 light flickering” → contact.)' },
