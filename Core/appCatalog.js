...
const PRESETS = [
  ...
  },
  {
    ...
    baseline: [
      ...
      // v2.74.2090 (critical-review content fixes #4/#7/#8/#9/#11/#12 folded in) — see logs/run/findings.md.
      { kind: 'delta', body: 'WARRANTY SWITCH — which product. THIS QUEUE IS DEAKO’S OWN VENDOR QUEUE: every task in it is already a Deako warranty task, so the word “Deako” does NOT have to appear in the note. A switch matter is Deako’s by SUBJECT — never ask the homeowner to confirm that a switch is “a Deako product”, and never escalate because the note didn’t name the brand. Whether a task is out of scope is decided by its SUBJECT (outlets, HVAC, plumbing, trim/paint), not by the presence of the word “Deako”. Deako is responsible for the SWITCHES on every warranty task; a described switch problem (sticking / flickering / dead / buzzing / loose / not working / needs replacing) is a REPLACEMENT — UNLESS the note negates or closes it (“do NOT replace”, “declined”, “already replaced/sent”, “fine now”, “leave as-is”, “cosmetic only”, “punch item closed”): that is NOT a replacement. THE DEFAULT PRODUCT IS THE SIMPLE ROCKER SWITCH (Single-Pole & Multiway), and it is the answer for EVERY ordinary switch wording: “switch”, “switches”, “light switch”, “wall switch”, “rocker switch”, “simple rocker”, “single-pole switch”, “multiway switch”, “3-way switch”. Those are ALL the same one product — the rocker covers single-pole AND multiway, so a 3-way is never downgraded, and “rocker switch” IS this product by name. NONE of those wordings is ambiguous and NONE of them is a reason to ask the homeowner: never send a task to CONTACT because the note said “light switches” or “rocker switch” rather than the full catalog name. ONLY THREE THINGS COUNT AS A DIFFERENT PRODUCT: a dimmer, a smart plug, or a smart switch (e.g. “Gen 2 smart switch”). Resolve those by their own name and never substitute the rocker for them; if such a named product cannot be resolved in the catalog, do not fall back to the rocker and do not dead-end — that is CONTACT HOMEOWNER (confirm the exact product). A “LIGHT” SYMPTOM IS A SWITCH FAULT — the homeowner describes what they see, and Deako ships the SWITCH that causes it: “1 light flickering — pls ship replacement”, “the light in the primary bath won’t come on”, “lights flicker when you push the switch”, “2 lights on the same switch will not work” are all REPLACE (count from the note; a singular light → 1). Never send one of these to CONTACT to “confirm switch vs fixture” — a light fault in this queue is Deako’s switch. The only light-shaped exception is a note that is explicitly about the BULB or the FIXTURE ITSELF with no switch involvement (“replace the broken fixture”, “bulb burned out”) — and that is NOT DEAKO (a subject that isn’t ours), never a contact. Worked examples: “light switch sticking” → 1 Simple Rocker Switch · “Need 4 light switches” → 4 Simple Rocker Switches · “one rocker switch for office, one for master bath” → 2 Simple Rocker Switches · “6 wall switches flickering” → 6 Simple Rocker Switches · “3-way switch doesn’t work” → 1 Simple Rocker Switch · “Gen 2 smart switch” → the smart switch product · “prim bath 1 light flickering — pls ship replacement” → 1 Simple Rocker Switch (a light symptom is a switch fault, never a contact).' },
      { kind: 'delta', body: 'WARRANTY SWITCH — how many: an explicit number wins (“4 light switches” → 4; “(6) total” → 6). A single switch problem with no number (“a light switch sticking”, “the switch is dead”) → 1. A DEFINITE quantifier is a count: “both” / “the two” / “a pair” → 2. A RANGE takes the UPPER BOUND and is a normal REPLACE, never a contact: “4-5” → 5, “2 or 3” → 3, “3 to 4” → 4 (over-shipping one switch is far better than leaving the homeowner short — the draft is reviewed before it goes out). An enumeration across places (“one for the office, one for the master bath”, “one in each of 3 bedrooms”) → the SUM; if a distributive has no multiplier you can read from the task (“one per bedroom”, bedroom count unknown), the total is unclear. Read the WHOLE instruction for the count, not just the line the word “switch” appears on (the number often sits on a nearby line, e.g. “Multiple deakos … 4-5”), and never take a number that is not counting switches (a lot/unit number, a date, a dollar amount, a model/generation like “Gen 2”, or a wiring type like “3-way”). If the count is genuinely unclear — an unreadable distributive, or a bare INDEFINITE plural with NO number anywhere (“send switches”, “multiple”, “several”) — do NOT invent a number; that is the CONTACT-HOMEOWNER disposition. (A range is NOT unclear: take the upper bound.)' },
      { kind: 'delta', body: 'WARRANTY SWITCH — disposition. WHEN YOU CLASSIFY, SORT, GROUP OR BRANCH WARRANTY TASKS, USE EXACTLY THESE THREE ARMS, WITH THESE LABELS — “replacement needed”, “contact homeowner”, “not deako” — and NEVER invent your own arms (never “requesting replacement / not requesting replacement”, never a two-way split). Every task belongs to one of them: “no arm” is a bug, not an answer. A REPORTED SWITCH PROBLEM IS A REPLACEMENT — the note does NOT have to ask for one: “light switch in the master bedroom is not working”, “switch quit working”, “lights flicker when you push the switch”, “switch is bad” are all REPLACE, exactly like “please deliver one switch”. Never file a switch fault under “no replacement requested”. A SINGULAR switch problem with NO number is REPLACE 1 — never a contact: “light switch sticking at middle bedroom”, “the switch is dead”, “bathroom switch is not working”, “switch in the garage causes flickering” all mean ONE switch, so the count is known and the task is REPLACE 1. “No count specified” is a contact reason ONLY for an INDEFINITE PLURAL (“send switches”, “multiple deakos”, “several switches” with no number anywhere) — a singular fault is never count-unclear. Every warranty task is exactly ONE of three (v1 handles ONE switch line per task; if a task orders DIFFERENT products, e.g. 2 switches AND 1 dimmer, say so plainly and propose the lines you can, never silently drop one): (1) REPLACE — a Deako switch matter with a clear count AND a resolved product → propose the draft order (count × the resolved product: the Simple Rocker Switch by default, or the specified dimmer / smart plug / smart switch); (2) CONTACT HOMEOWNER — a Deako switch matter but the count, the product, or the subject is genuinely unclear (an indefinite plural with no number anywhere, a NAMED product that cannot be resolved, — but NOT a range, which takes its upper bound, and NOT a light symptom, which is a switch fault → REPLACE) → propose contacting the homeowner for clarity, never draft a guessed order. An ORDINARY switch wording is NOT unclear — “switch(es)”, “light switch”, “wall switch”, “rocker switch”, “single-pole”, “3-way” with a count all take the DEFAULT product and belong in REPLACE. Product-identity doubt is a CONTACT reason ONLY for an unresolvable dimmer / smart plug / smart switch; never for the ordinary wordings, and never merely because the note did not use the full catalog name; (3) NOT DEAKO — decided by SUBJECT, never by whether the note says “Deako”: electrical outlets, HVAC, plumbing, trim/paint, or a note that is ONLY routing/scheduling with no switch order under it → out of scope, route or skip. A switch task that never mentions the brand is still IN scope (this is Deako’s own queue). PRECEDENCE: a real switch order DOMINATES an accompanying scheduling/ASAP/routing note — “replacement needed for 3 switches, SCHEDULE ASAP” is REPLACE, not NOT DEAKO. Never force an unclear or non-switch task into REPLACE, and never drop a real switch order because a schedule note rode along.' },
    ],
  },
  {
