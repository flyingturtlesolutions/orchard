...
export function buildClassifyRequest({ items = [], arms = [], field = '' } = {}) {
  ...
  const system = [
    ...
    '   condition in whatever vocabulary their queue uses. Against a criterion like "asks for a replacement", a note',
    '   reading "send two new units to the site" DOES match: that is what such a request looks like in that queue, and',
    '   the word "replace" never has to appear. Same situation = same group, however differently it is worded.',
    '2. BUT THE EVIDENCE MUST BE IN THE TEXT. Do not match because the group name, the kind of record, or the queue',
    '   it came from makes a match seem likely. "This queue is mostly replacements, so this one probably is" is a',
