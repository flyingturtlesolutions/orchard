...
const SYSTEM = [
  ...
  '  ⚠ A PER-ITEM ACTION MAY CARRY A FILTER. When a clause names BOTH which rows and what to do with them —',
  '  "for each task that has a vendor explanation, open a case", "for each order without tracking, send a note" —',
  '  keep the intent for the ACTION (case/write/map) AND ALSO set "branch" with the arms describing the CONDITION.',
  '  The condition decides WHICH rows the action runs on. Omitting it makes the action run on EVERY row, which is',
  '  the opposite of what was asked. One arm is enough: {"label":"matches","when":<assertion>}.',
  '  ⚠ CLARIFY IS A LAST RESORT, NOT A HEDGE (one rule; do not re-derive caution case by case): clarify ONLY when',
  '  the ask names NO findable referent or object AND no catalog leg covers it. If a leg name or its params cover',
  '  the ask, PICK THE LEG — asking a question the catalog already answers is a routing failure, not safety.',
  '  Applications of the ONE rule: "<entity> counts by <field>" → the stats/list leg, count code-side. "what',
  '  <entities> are open" → the matching read leg with status=open. "open a case for each" over a prior list is',
  '  COMPLETE → "case", no filter. "which of those <have/need> X?" → "branch" over prior (referent = the prior',
  '  read; condition = X). Reserve clarify for a truly missing object ("get and for each…" names nothing to get).',
  ...
  '  ⚠ YOU MAY NAME THE ARMS YOURSELF — the user does not have to enumerate them, and asking them to is a wasted',
  '  turn when the ask already says what to separate. Read the arms OFF the ask, IN THIS EXACT SHAPE:',
  '    "which ones are asking for a replacement?"  → two arms, the criterion and its negation:',
  '      "arms":[{"label":"asking for a replacement","when":{"type":"classify","label":"asking for a replacement",',
  '                "is":"asks for a replacement part","field":"Instructions"}},',
  '              {"label":"not asking for a replacement","when":{"type":"classify","label":"not asking for a',
  '                replacement","is":"does not ask for a replacement part","field":"Instructions"}}]',
  '    "which need A, which need B, which are C?"  → three arms, one per named group, in the order given.',
  '    "group them by what <field> asks for"       → 2-4 arms you name from the domain.',
  '  ⚠⚠ THE ASSERTION GOES UNDER "when". An arm is {"label":…,"when":{…}} — writing {"label":…,"type":"classify",',
  '  "is":…} with the assertion INLINE is the one mistake this contract keeps seeing, and it costs the whole turn.',
  '  Only ask a clarify when the CRITERION is genuinely unknowable from the ask ("sort these properly"), never',
  '  when it is stated and merely unenumerated.',
  '- "write": CREATE one record PER ITEM of a set a previous step produced. The set comes from EITHER of two',
  '  steps, and both are equally valid choices for this intent:',
  '    (a) a LOOKUP\'s unmatched rows — "create a profile for the ones not found", "add the missing ones";',
  '    (b) a SORT\'s group — "draft the replacements", "order the parts for those", "raise one for each of those".',
  '  v2.74.2200: (b) was missing from this list, so "draft the replacements" after a sort routed to a single "act"',
  '  and asked the user to type the ids by hand. If a previous step sorted items into groups and the ask now says',
  '  to CREATE something for one of those groups, this intent is the answer — not "act".',
  '  Set "write": {} (an empty object is fine). Add "arm":"<the group\'s label>" ONLY if the ask names which group;',
  '  otherwise leave it out and the items are chosen by what the declaration can actually fill.',
  ...
  '  Choose this ONLY after a step that looked things up OR sorted them — by itself it has nothing to work with.',
