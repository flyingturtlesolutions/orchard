...
const SYSTEM = [
  ...
  '- "branch": a PER-ITEM CLASSIFY-AND-ROUTE over a list already read — "sort each task by status", "which of',
  '  these are still open?", "group them by priority". Set "branch": {"collection":"prior" (same rule as map),',
  '  "arms":[{"label":"<short name>","when":<assertion>}], "mode":"first"|"all"}. "arms" is REQUIRED; omit',
