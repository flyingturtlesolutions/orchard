...
export function recipeFromCatalogEntry(entry, { groundId = '', origin = '' } = {}) {
  ...
  for (const k of ['itemUrl', 'listUrl', 'bodyType', 'contentType', 'identityProbe', 'probeAccept', 'persistedOp', 'csrf', 'csrfHeader', 'csrfCookie', 'retention', 'apiHost', 'listPath', 'autoRequires', 'capClass', 'displayId', 'joinKey', 'writeMap', 'display']) {   // DK-2 — capClass:'presence' survives the seeded path too; CP-1 — probeAccept (json-liveness) rides the seeded path; CX-9k — displayId (the human row-id keys) rides too (Invariant #3 hop 1)
