// Core/ilRun.js — compose the inference-layer loop into one runnable (DESIGN_inference_layer.md §4). IL-2 (v2.74.1111).
//
// PURE-with-injected-deps: this is the GLUE that turns the four separate cores (palette · agentLoop · execPlan ·
// the think seam) into a single `runIl(goal, ctx, deps)`. Everything impure is a dep:
//   • retrieve(goal,k)  — the learned tool-RAG source (R-2) for assemblePalette
//   • il(stepCtx)    — the think seam (AnthropicService.stepIl)
 ...
 *   il?:    (stepCtx:object)=>Promise<object>,            // → Decision
 ...
 */
export async function runIl(goal, ctx = {}, deps = {}, opts = {}) {
  const { retrieve, il, exec, outcomes, rules, env, verifyDone, isAborted } = deps || {};
  ...
  // Orchard re-engages instead of the loop dying.
  ...
  return agentLoop(goal, { palette, callIl: il, runTool, verifyDone, isAborted }, opts);
}
