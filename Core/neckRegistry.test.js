...
describe('neckRegistry — row discipline', () => {
  ...
  it('ALL NINE routing-grade necks are gated; the owed list is EMPTY (v1734 — Stage 7 complete)', () => {
    assert.deepEqual(necksByGate('built').map((n) => n.operation).sort(),
      ['branch-classify', 'decompose-steps', 'interpret', 'judge-match', 'match-workflow', 'route-ask', 'seed-directives', 'step-il', 'sweep-reads']);
